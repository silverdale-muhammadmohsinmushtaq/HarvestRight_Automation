...
test.describe.serial('Odoo End-to-End QA', () => {
  ...
  await page.getByRole('button', { name: 'Update' }).click();
  await expect(page.getByRole('alert')).toContainText('Contact is valid and validated');
  await expect(page.locator('body')).toContainText('7842 BIG TIMBER TRL');
  await expect(page.locator('body')).toContainText('MIDDLETON');
  await expect(page.locator('body')).toContainText('Wisconsin (US)');
  await page.getByText('-4159').click();
  await expect(page.locator('body')).toContainText('53562');
  ...
  await expect(page.locator('body')).toContainText('**Test Customer 001**');
